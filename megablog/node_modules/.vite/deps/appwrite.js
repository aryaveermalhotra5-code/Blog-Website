...
var Client = class Client {
		...
		*/
		...
		this.realtime = {
			...
			createSocket: () => {
				...
				const queryParams = "project=" + encodeURIComponent((_b = this.config.project) !== null && _b !== void 0 ? _b : "");
				...
				} else if (((_e = this.realtime.socket) === null || _e === void 0 ? void 0 : _e.readyState) === WebSocket.OPEN) this.realtime.sendPendingSubscribes();
			},
			sendPendingSubscribes: () => {
				if (!this.realtime.socket || this.realtime.socket.readyState !== WebSocket.OPEN) return;
				if (this.realtime.pendingSubscribes.size < 1) return;
	...
	*/
	subscribe(channels, callback, queries = []) {
		...
		if (subscriptionId === "") throw new AppwriteException("Failed to generate unique subscription id");
		...
		return () => {
			...
			this.realtime.channels.forEach((channel) => {
				if (!stillUsed.has(channel)) this.realtime.channels.delete(channel);
			});
			this.realtime.connect();
		};
	}
	...
	chunkedUpload(method_1, url_1) {
		return __awaiter(this, arguments, void 0, function* (method, url, headers = {}, originalPayload = {}, onProgress) {
			...
			if (totalChunks === 1) return response;
			const chunks = [];
			...
			const isUploadComplete = (chunkResponse) => {
				...
				const chunksTotal = (_a = chunkResponse === null || chunkResponse === void 0 ? void 0 : chunkResponse.chunksTotal) !== null && _a !== void 0 ? _a : totalChunks;
				return typeof chunksUploaded === "number" && typeof chunksTotal === "number" && chunksUploaded >= chunksTotal;
			};
			const uploadChunk = (chunk) => __awaiter(this, void 0, void 0, function* () {
				const chunkHeaders = Object.assign({}, headers);
				if (uploadId) chunkHeaders["x-appwrite-id"] = uploadId;
				...
				if (rejected) return chunkResponse;
				completedCount++;
				uploadedBytes += chunk.end - chunk.start;
				lastResponse = chunkResponse;
				if (isUploadComplete(chunkResponse)) finalResponse = chunkResponse;
				if (onProgress && typeof onProgress === "function") onProgress({
					$id: uploadId,
	...
	ping() {
		return __awaiter(this, void 0, void 0, function* () {
			return this.call("GET", new URL(this.config.endpoint + "/ping"), { "X-Appwrite-Project": this.config.project });
		});
	}
	call(method_1, url_1) {
		return __awaiter(this, arguments, void 0, function* (method, url, headers = {}, params = {}, responseType = "json") {
			...
			if (data && typeof data === "object") Object.defineProperty(data, "toString", {
...
var Presences = class {
	...
	get(paramsOrFirst) {
		let params;
		if (paramsOrFirst && typeof paramsOrFirst === "object" && !Array.isArray(paramsOrFirst)) params = paramsOrFirst || {};
		else params = { presenceId: paramsOrFirst };
		const presenceId = params.presenceId;
		if (typeof presenceId === "undefined") throw new AppwriteException("Missing required parameter: \"presenceId\"");
	...
	update(paramsOrFirst, ...rest) {
		...
		if (typeof metadata !== "undefined") payload["metadata"] = metadata;
		if (typeof permissions !== "undefined") payload["permissions"] = permissions;
		if (typeof purge !== "undefined") payload["purge"] = purge;
		const uri = new URL(this.client.config.endpoint + apiPath);
		const apiHeaders = {
			"X-Appwrite-Project": this.client.config.project,
			"content-type": "application/json",
...
var Realtime = class {
	constructor(client) {
		...
		this.pendingSubscribes = /* @__PURE__ */ new Map();
	...
	*/
	...
	startHeartbeat() {
		...
		this.heartbeatTimer = window === null || window === void 0 ? void 0 : window.setInterval(() => {
			if (this.socket && this.socket.readyState === WebSocket.OPEN) this.socket.send(JSONbig.stringify({ type: "ping" }));
		}, this.HEARTBEAT_INTERVAL);
	}
	stopHeartbeat() {
		if (this.heartbeatTimer) {
			window === null || window === void 0 || window.clearInterval(this.heartbeatTimer);
	...
	*/
	createSocket() {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (this.socketCreationPromise) return this.socketCreationPromise;
	...
	generateUniqueSubscriptionId() {
		...
		for (let i = 0; i < attempts; i++) {
			const id = ID.unique();
			if (!this.activeSubscriptions.has(id)) return id;
		}
	...
	*/
	...
	sendPendingSubscribes() {
		if (!this.socket || this.socket.readyState !== WebSocket.OPEN) return;
		if (!this.appConnected) return;
		if (this.pendingSubscribes.size < 1) return;
		...
		return __awaiter(this, arguments, void 0, function* (channelsOrChannel, callback, queries = []) {
			...
			try {
				...
				if (this.subCallDepth === 1) {
					if (!this.socket || this.socket.readyState > WebSocket.OPEN) yield this.createSocket();
					else if (this.socket.readyState === WebSocket.OPEN) this.sendPendingSubscribes();
				}
			} finally {
				this.subCallDepth--;
			}
			...
			const update = (changes) => __awaiter(this, void 0, void 0, function* () {
				const subscription = this.activeSubscriptions.get(subscriptionId);
				if (!subscription) return;
				if (changes.channels !== void 0) {
					const nextChannelStrings = changes.channels.map((ch) => this.channelToString(ch));
					subscription.channels = new Set(nextChannelStrings);
				}
				if (changes.queries !== void 0) {
					const nextQueries = [];
					for (const q of changes.queries) if (Array.isArray(q)) for (const inner of q) nextQueries.push(typeof inner === "string" ? inner : inner.toString());
					else nextQueries.push(typeof q === "string" ? q : q.toString());
	...
	*/
	upsertPresence(params) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (params.permissions !== void 0) data.permissions = params.permissions;
			if (params.metadata !== void 0) data.metadata = params.metadata;
			this.pendingPresence = data;
			if (!this.socket || this.socket.readyState >= WebSocket.CLOSING) this.createSocket().catch((error) => {
	...
	handleResponseConnected(message) {
		...
		if (session && !messageData.user) (_b = this.socket) === null || _b === void 0 || _b.send(JSONbig.stringify({
			type: "authentication",
			data: { session }
		}));
		for (const subscriptionId of this.activeSubscriptions.keys()) this.enqueuePendingSubscribe(subscriptionId);
...
//#endregion
export { Account, AppwriteException, AuthenticationFactor, AuthenticatorType, Avatars, Browser, BrowserPermission, BrowserTheme, Channel, Client, Condition, CreditCard, Databases, ExecutionMethod, ExecutionStatus, ExecutionTrigger, Flag, Functions, Graphql, ID, ImageFormat, ImageGravity, Locale, Messaging, OAuthProvider, Operator, Permission, Presences, Query, Realtime, Role, Storage, TablesDB, Teams, Timezone };
